...
async def load_config():
    ...
    info.DATABASE_URI = data.get("DATABASE1") or info.DATABASE_URI
    info.DATABASE_URI2 = data.get("DATABASE2") or info.DATABASE_URI2
    info.DATABASE_URI3 = data.get("DATABASE3") or info.DATABASE_URI3
    info.ADMINS = data.get("ADMINS", info.ADMINS)
...
async def update_sub_buttons(client, message, sh):
    ...
    for _ in range(SET_SYDSEC * 2):
        ...
        for ch in filter(None, channels):
            if ch not in cache:
                try:
                    cache[ch] = (await client.create_chat_invite_link(
    ...
    for ch in auth:
        if ch not in joined and not await is_mem(ch):
            ...
            if len(pending) == 2:
                break
    if pending:
        return False, pending[0], pending[1] if len(pending) > 1 else None

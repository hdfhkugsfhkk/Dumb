...
from utils import get_settings, get_size, is_subscribed, update_sub_buttons, get_authchannel, save_group_settings, temp, check_loop_sub, check_loop_sub1, check_loop_sub2, is_authorized, auth_required, send_alert_to_admins
...
async def send_file(client, query, ident, file_id):
    ...
    if f_caption is None:
        f_cation = f"{title}"
    inline_keyboard = [[
            InlineKeyboardButton('🖥 Oᴛᴛ Uᴩᴅᴀᴛᴇꜱ Cʜᴀɴɴᴇʟ 🖥', url=f'https://t.me/CGM_Files')
            ],[     
            InlineKeyboardButton('⚙ Lᴀᴛᴇꜱᴛ Mᴏᴠɪᴇ Rᴇʟᴇᴀꜱᴇꜱ ⚙', url='https://t.me/CGM_Files')
    ]]
    ...
    if FSUB:
        try:
            ...
            if not fsub:
                ...
                syyd=await client.send_message(
                ...
                syd = await update_sub_buttons(client, message, syyd)
                if not syd:
                    return
        except Exception as e:
